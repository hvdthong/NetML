
















org apach common math3 od nonstiff











interpol gragg bulirsch stoer
integr

interpol comput dens output insid step
produc gragg bulirsch stoer integr


implement basic reimplement java

href http www unig math folk hairer prog nonstiff odex odex
fortran code hairer wanner redistribut polici
code
href http www unig hairer prog licenc txt
conveni reproduc


tabl border width cellpad align center bgcolor e0e0e0
copyright ernst hairer

redistribut sourc binari form
modif permit provid
condit met

redistribut sourc code retain copyright
notic list condit disclaim
redistribut binari form reproduc copyright
notic list condit disclaim
document materi provid distribut


strong softwar provid copyright holder
contributor express impli warranti includ
limit impli warranti merchant fit
purpos disclaim event regent
contributor liabl direct indirect incident special
exemplari consequenti damag includ limit
procur substitut good servic loss data
profit busi interrupt caus theori
liabil contract strict liabil tort includ
neglig aris
softwar advis possibl damag strong
tabl

gragg bulirsch stoer integr graggbulirschstoerintegr
version



gragg bulirsch stoer step interpol graggbulirschstoerstepinterpol











































































































































































































































inherit doc inheritdoc
overrid
comput interpol state deriv computeinterpolatedstateandderiv theta
minu theta oneminusthetah

dimens current state currentst length

minu theta oneminustheta theta
theta05 theta
tomt theta minu theta oneminustheta
tomt tomt
dot t4dot tomt theta
dot1
dot2 theta theta
dot3 theta theta

dimens

polynomi
polynomi
polynomi
polynomi
interpol state interpolatedst theta minu theta oneminustheta theta minu theta oneminustheta
interpol deriv interpolatedderiv dot1 dot2 dot3

current degre currentdegre
dot cdot
polynomi current degre currentdegre
current degre currentdegre

dot cdot theta05 dot cdot
polynomi theta05

interpol state interpolatedst
interpol deriv interpolatedderiv dot cdot dot t4dot





degener previou comput lead nan deriv
fix deriv midpoint
system arraycopi mid dot ymiddot interpol deriv interpolatedderiv dimens


















































