














































































































org jfree chart render



































standard item render link plot xyplot draw
shape point line point shape
line

render retain histor reason gener
link line shape render xylineandshaperender

standard item render standardxyitemrender abstract item render abstractxyitemrender





















































































































































































































































































































































































































































































































































































draw visual represent singl data item

param graphic devic
param state render state
param data area dataarea area data drawn
param plot plot obtain standard color inform

param domain axi domainaxi domain axi
param rang axi rangeaxi rang axi
param dataset dataset
param seri seri index base
param item item index base
param pass pass index

draw item drawitem graphics2 graphics2d item render state xyitemrendererst state
rectangle2 rectangle2d data area dataarea plot xyplot plot axi valueaxi domain axi domainaxi
axi valueaxi rang axi rangeaxi dataset xydataset dataset seri item
select pass

item visibl itemvis item visibl getitemvis seri item

setup collect option entiti info
shape entiti area entityarea
entiti collect entitycollect entiti
state info getinfo
entiti state info getinfo owner getown entiti collect getentitycollect


plot orient plotorient orient plot orient getorient
paint paint item paint getitempaint seri item select
stroke seri stroke seriesstrok item stroke getitemstrok seri item select
set paint setpaint paint
set stroke setstrok seri stroke seriesstrok

data point
dataset getxvalu seri item
dataset getyvalu seri item
doubl isnan doubl isnan
item visibl itemvis


rectangl edg rectangleedg axi locat xaxisloc plot domain axi edg getdomainaxisedg
rectangl edg rectangleedg axi locat yaxisloc plot rang axi edg getrangeaxisedg
tran transx1 domain axi domainaxi java2 valuetojava2d data area dataarea axi locat xaxisloc
tran transy1 rang axi rangeaxi java2 valuetojava2d data area dataarea axi locat yaxisloc

plot line getplotlin
draw seri line path drawserieslineaspath
state state state
seri index getseriesindex seri
start seri path
seri path seriespath reset
point good lastpointgood
set seri index setseriesindex seri


updat path reflect latest point
item visibl itemvis doubl isnan tran transx1
doubl isnan tran transy1
tran transx1
tran transy1
orient plot orient plotorient horizont
tran transy1
tran transx1

point good islastpointgood
todo check threshold
seri path seriespath line lineto


seri path seriespath move moveto

set point good setlastpointgood


set point good setlastpointgood

item dataset item count getitemcount seri
seri index seriesindex seri
draw path
set stroke setstrok lookup seri stroke lookupseriesstrok seri
set paint setpaint lookup seri paint lookupseriespaint seri
draw seri path seriespath




item item visibl itemvis
previou data point
dataset getxvalu seri item
dataset getyvalu seri item
doubl isnan doubl isnan
draw line drawlin
plot discontinu getplotdiscontinu
draw line gap current
previou data point threshold
num numx dataset item count getitemcount seri
min minx dataset getxvalu seri
max maxx dataset getxvalu seri num numx
gap threshold type gapthresholdtyp unit type unittyp absolut
draw line drawlin math ab gap threshold gapthreshold


draw line drawlin math ab max maxx min minx
num numx gap threshold getgapthreshold


draw line drawlin
tran transx0 domain axi domainaxi java2 valuetojava2d data area dataarea
axi locat xaxisloc
tran transy0 rang axi rangeaxi java2 valuetojava2d data area dataarea
axi locat yaxisloc

draw good valu
doubl isnan tran transx0 doubl isnan tran transy0
doubl isnan tran transx1 doubl isnan tran transy1



orient plot orient plotorient horizont
state work line workinglin set line setlin tran transy0 tran transx0
tran transy1 tran transx1

orient plot orient plotorient vertic
state work line workinglin set line setlin tran transx0 tran transy0
tran transx1 tran transy1


state work line workinglin intersect data area dataarea
draw state work line workinglin






need invis item ensur
seri path seriespath updat happen
visibl item
item visibl itemvis



base shape visibl getbaseshapesvis

shape shape item shape getitemshap seri item select
orient plot orient plotorient horizont
shape shape util shapeutil creat translat shape createtranslatedshap shape tran transy1
tran transx1

orient plot orient plotorient vertic
shape shape util shapeutil creat translat shape createtranslatedshap shape tran transx1
tran transy1

shape intersect data area dataarea
item shape fill getitemshapefil seri item
fill shape


draw shape


entiti area entityarea shape



plot imag getplotimag
imag imag imag getimag plot seri item tran transx1 tran transy1
imag
point hotspot imag hotspot getimagehotspot plot seri item tran transx1
tran transy1 imag
draw imag drawimag imag tran transx1 hotspot getx
tran transy1 hotspot geti
entiti area entityarea rectangle2 rectangle2d doubl tran transx1 hotspot getx
tran transy1 hotspot geti imag width getwidth
imag height getheight




tran transx1
tran transy1
orient plot orient plotorient horizont
tran transy1
tran transx1


draw item label
item label visibl isitemlabelvis seri item select
draw item label drawitemlabel orient dataset seri item select



domain axi index domainaxisindex plot domain axi index getdomainaxisindex domain axi domainaxi
rang axi index rangeaxisindex plot rang axi index getrangeaxisindex rang axi rangeaxi
crosshair state xycrosshairst crosshair state crosshairst state crosshair state getcrosshairst
updat crosshair valu updatecrosshairvalu crosshair state crosshairst domain axi index domainaxisindex
rang axi index rangeaxisindex tran transx1 tran transy1 orient

add entiti item
entiti
shape util shapeutil point rect ispointinrect data area dataarea
add entiti addent entiti entiti area entityarea dataset seri item select





















































































































































